...
import { EXECUTABLE_PATH, EXECUTABLE_URL } from './src/constants';

/**
 * Check NODE_ENV value to determine what executable to download.
 */
function downloadStandaloneBinary() {
    ...
    if (existsSync(EXECUTABLE_PATH)) {
        console.log(process.env.NODE_ENV, 'Post-install: Download skipped, executable already exists.');
        return;
    }
    const executableDownloadPartPath = `${EXECUTABLE_PATH}.part`;
    const file = createWriteStream(executableDownloadPartPath);
    console.log(`${process.env.NODE_ENV} - Post-install: Downloading standalone binary... ${EXECUTABLE_URL}`);
